describe("Login End-to-End Tests", () => {
  ...
  describe("Login Page - UI and Navigation", () => {
    it("should display the login page correctly", () => {
      ...
      cy.contains("Inicia sesión para acceder a tu cuenta").should("be.visible");
  ...
  describe("Login Flow - Without CallbackUrl", () => {
    ...
    it("should preserve form submission behavior", () => {
      ...
      // Verificar que el botón está dentro del formulario
      cy.get("form").within(() => {
        cy.get('button[type="submit"]').contains("Login with Google").should("exist");
      });
    });
  ...
  describe("Login Flow - With CallbackUrl", () => {
    it("should preserve callback URL in the login form", () => {
        ...
        cy.url().should("include", `callbackUrl=${encodeURIComponent(callbackUrl)}`);
    });
  ...
  describe("Login Integration with Navbar", () => {
    ...
    it("should not show login button after successful authentication", () => {
        ...
        cy.log("Expected behavior: When user is authenticated, Login button should be hidden");
        cy.log("Current state: User is not authenticated, so Login button is visible");
  ...
  describe("Security Considerations", () => {
    it("should clear sensitive data on logout", () => {
      ...
      cy.window().its("localStorage").invoke("getItem", "nextauth.session-token").should("be.null");
    });
